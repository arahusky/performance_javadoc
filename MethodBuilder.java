...
package cz.cuni.tools.doclets.internal.toolkit.builders;
...
public class MethodBuilder extends AbstractMemberBuilder {
    ...
    public void buildMethodDoc(XMLNode node, Content memberDetailsTree) {
        ...
        //gets the number of methods in specified class
        int size = methods.size();
        if (size > 0) {
            //just writes the <h3> with Method detail
    ...
    /**
     * Build the performance information.
     *
     * @param node the XML element that specifies which components to document
     * @param methodDocTree the content tree to which the documentation will be added
     */
    public void buildPerformanceInfo(XMLNode node, Content methodDocTree)
    {
        writer.addPerformance((MethodDoc) methods.get(currentMethodIndex),
                methodDocTree);
        System.out.println("=================");
